...
describe("Round010 grounded sword-contact pose beat", () => {
  ...
  it("carries force through exterior contact, same-direction overshoot, and target recoil", () => {
    ...
    expect(contact.grip.weaponMountRotation[2]).toBe(-0.012);
    expect(coil.grip.weaponMountRotation).toEqual([0, 0, 0]);
    expect(recoil.grip.weaponMountRotation).toEqual([0, 0, 0]);
    ...
    expect(targetContact.model.position[0]).toBeLessThanOrEqual(-0.71);
    expect(targetContact.bones.upperArmL[2]).toBeGreaterThan(0.1);
    expect(targetContact.bones.upperArmR[2]).toBeLessThan(-0.2);
