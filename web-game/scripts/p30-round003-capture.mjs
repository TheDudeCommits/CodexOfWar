...
const WIDTH = 1600;
const HEIGHT = 900;
const BASE_URL = process.env.ROUND003_URL ?? "http://127.0.0.1:4173/?review=1&post=0";
const OUTPUT_ROOT = resolve(process.cwd(), "../ArtSource/P30/Round003");
const CHROME = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const headless = process.env.ROUND003_HEADLESS === "1";
...
function parsePng(data) {
  ...
  const forbiddenMetadata = chunks.filter((type) =>
...
async function advanceScenario(page, scenario) {
  return page.evaluate(async ({ preset, ticks, actions }) => {
...
async function runTape(page, kind) {
  return page.evaluate(async (tapeKind) => {
    const review = window.__COW_REVIEW__;
...
const evidence = {
  schema: "p30.round003.production-capture-evidence.v1",
  candidateOnly: true,
