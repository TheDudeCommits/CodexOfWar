...
async function runSoak(page, out, captureFrames) {
  ...
  const attacksBefore = await page.evaluate(() => window.__P30_CRITIC__.runReceipt().eventLog
    .filter((event) => event.type === 'attack-started').length);
  await page.mouse.click(800, 450, { button: 'left' });
  let postSoakStrikeAcknowledged = false;
  try {
    await page.waitForFunction(
      (count) => window.__P30_CRITIC__.runReceipt().eventLog
        .filter((event) => event.type === 'attack-started').length > count,
      attacksBefore,
...
async function main() {
  ...
  try {
    ...
    const updateInputs = new Map(runReceipt.fixedInputHistory.map((item) => [item.sampledUpdateTick, item]));
    const traceValidation = {
      wTicks0Through19: Array.from({ length: 20 }, (_, tick) => updateInputs.get(tick)?.moveZ === -1_000_000).every(Boolean),
      idleTicks20Through23: [20, 21, 22, 23].every((tick) => {
        const input = updateInputs.get(tick);
        return input?.moveX === 0 && input?.moveZ === 0;
      }),
      exactlyOneMouseRisingEdgeAtAbsolute24: runReceipt.inputEdgeLog.length === 1
        && runReceipt.inputEdgeLog[0].action === 'light-strike'
        && runReceipt.inputEdgeLog[0].phase === 'rising'
        && runReceipt.inputEdgeLog[0].device === 'mouse'
        && runReceipt.inputEdgeLog[0].button === 'left'
        && runReceipt.inputEdgeLog[0].absoluteSimulationTick === 24
        && runReceipt.inputEdgeLog[0].attackRelativeTick === 0
    };
    ...
    if (!Object.values(traceValidation).every(Boolean)) fail(`locked input trace validation failed: ${JSON.stringify(traceValidation)}`);
